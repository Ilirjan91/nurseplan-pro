...
function paintCell(rowIndex, dayKey, cell) {
  ...
  row.actual = roundOne(numeric(row.actual) - hoursForCode(previous) + hoursForCode(next));
...
function renderTable() {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    days.forEach((day) => {
      ...
      const td = document.createElement("td");
      td.className = `np-cell ${codeClass(value)}${day.weekend ? " weekend" : ""}${locked ? " locked" : ""}`;
