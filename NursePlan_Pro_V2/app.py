...
from solver_v2 import OrToolsUnavailableError, blocked_absence_dates, solve_month_v2
...
def _validate_plan_impl(plan, employees, settings):
    ...
    def consecutive_blocks(values: set[date]) -> list[list[date]]:
        blocks: list[list[date]] = []
        for value in sorted(values):
            if not blocks or value != blocks[-1][-1] + timedelta(days=1):
                blocks.append([value])
            else:
    ...
    if regel_aktiv(settings, "regel_nachtblock_aktiv"):
        min_night_block = max(1, int(settings.get("min_nachtblock", 2)))
        if regel_aktiv(settings, "regel_max_nachtdienste_aktiv"):
            min_night_block = min(max_nights, min_night_block)
        free_after_nights = max(
            0,
            int(settings.get("freie_tage_nach_nachtblock", 2)),
        )
        for key, dates in night_dates.items():
            for block in consecutive_blocks(dates):
                if len(block) < min_night_block:
                    errors.append(
                        f'{employee_map[key]["name"]}: Einzelner oder zu kurzer Nachtblock '
                        f'ab {block[0].strftime("%d.%m.")} '
                        f'(mindestens {min_night_block} Nächte erforderlich).'
                    )
                blocked_days = {
                    block[-1] + timedelta(days=offset)
                    for offset in range(1, free_after_nights + 1)
                }
                if blocked_days & work_dates[key]:
                    errors.append(
                        f'{employee_map[key]["name"]}: Nach dem Nachtblock bis '
                        f'{block[-1].strftime("%d.%m.")} fehlen '
                        f'{free_after_nights} freie Tage.'
                    )
    ...
    for rule in eigene_planungsregeln(settings):
        ...
        if rule_type == "nicht_gemeinsam":
            ...
            for person in employees:
                ...
                if trigger_code == "N":
                    # Bei Nachtdiensten beginnt die freie Zeit erst nach dem
                    # vollständigen Block, nicht nach jeder einzelnen Nacht.
                    trigger_days = {
                        value for value in trigger_days
                        if value + timedelta(days=1) not in trigger_days
                    }
...
def _create_automatic_month_impl(year: int, month: int, employees, settings, vorgaben):
    ...
    blocked_absences = blocked_absence_dates(vorgaben)
    ...
    def violates_custom_rule(person: dict, entry: dict, code: str) -> bool:
        ...
        for rule in active_custom_rules:
            ...
            if rule_type == "nicht_gemeinsam":
                ...
                free_days = max(1, regel_zahl(rule, "freie_tage", 1))
                if trigger_code == "N":
                    candidate_trigger_days = set(service_dates[key].get("N", set()))
                    candidate_work_days = set(work_date_sets[key])
                    if code == "N":
                        candidate_trigger_days.add(day_value)
                    candidate_work_days.add(day_value)
                    block_ends = {
                        value for value in candidate_trigger_days
                        if value + timedelta(days=1) not in candidate_trigger_days
                    }
                    if any(
                        any(
                            block_end < work_day <= block_end + timedelta(days=free_days)
                            for work_day in candidate_work_days
                        )
    ...
    def can_assign(person: dict, entry: dict, code: str) -> bool:
        ...
        if day_value is None or targets[key] <= 0 or code_to_hours[code] <= 0:
            return False
        if (int(person.get("id", -1)), day_value) in blocked_absences:
            return False
...
def create_pdf(plan, plan_name, employees, settings):
    ...
    for column_index, column in enumerate(matrix.columns):
        if str(column).endswith((" Sa", " So")):
            table.setStyle(
                TableStyle(
                    [
                        ("BACKGROUND", (column_index, 0), (column_index, -1), colors.HexColor("#FFF4CC")),
                        ("TEXTCOLOR", (column_index, 0), (column_index, 0), colors.HexColor("#6F4B00")),
                        ("BOX", (column_index, 0), (column_index, -1), 0.45, colors.HexColor("#E2B93B")),
                    ]
                )
            )
...
def readable_plan_matrix(matrix: pd.DataFrame):
    ...
    weekend_columns = [
        column for column in day_columns
        if str(column).endswith((" Sa", " So"))
    ]
    ...
    styled = matrix.style
    if weekend_columns:
        weekend_header_styles = []
        for column in weekend_columns:
            column_index = matrix.columns.get_loc(column)
            weekend_header_styles.append(
                {
        ...
        styled = styled.set_table_styles(weekend_header_styles, overwrite=False)

        def style_weekend(value):
            is_empty = value is None or pd.isna(value) or not str(value).strip()
            background = "background-color: #fff8d9;" if is_empty else ""
            return (
                f"{background} border-left: 1px solid #e2b93b; "
                "border-right: 1px solid #e2b93b;"
            )

        styled = styled.map(style_weekend, subset=weekend_columns)
    if day_columns:
    ...
    with basic_tab:
        ...
        with st.form("basic_planning_rules_form"):
            ...
            with right:
                ...
                min_night_block = st.number_input(
                    "Minimale Nachtdienste je Block",
                    min_value=1,
                    max_value=10,
                    value=max(1, min(10, int(settings.get("min_nachtblock", 2)))),
                    step=1,
                    help="Verhindert einzelne Nachtdienste. Empfohlen: mindestens 2.",
                )
                free_after_night_block = st.number_input(
                    "Freie Tage nach einem Nachtblock",
                    min_value=0,
                    max_value=7,
                    value=max(
                        0,
                        min(7, int(settings.get("freie_tage_nach_nachtblock", 2))),
                    ),
                    step=1,
                    help="Die freien Tage beginnen erst nach der letzten Nacht des Blocks.",
                )
            ...
            with q1:
                ...
                night_block_active = st.checkbox(
                    "Nachtdienste nur in zusammenhängenden Blöcken planen",
        ...
        if (
            save_basic_rules
            and bool(night_block_active)
            and bool(max_nights_active)
            and int(min_night_block) > int(max_night_days)
        ):
            st.error(
                "Die Mindestlänge des Nachtblocks darf nicht größer als die maximale Anzahl "
                "aufeinanderfolgender Nachtdienste sein."
            )
        elif save_basic_rules:
            ...
            new_settings.update(
                {
                    ...
                    "min_nachtblock": int(min_night_block),
                    "freie_tage_nach_nachtblock": int(free_after_night_block),
    ...
    with overview_tab:
        ...
        basic_rows = [
            ...
            (
                "Nachtblock",
                "regel_nachtblock_aktiv",
                f'mind. {int(settings.get("min_nachtblock", 2))} Nächte, danach '
                f'{int(settings.get("freie_tage_nach_nachtblock", 2))} freie Tage',
            ),
