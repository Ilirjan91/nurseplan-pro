...
def manual_day_table(entry: dict, employees) -> tuple[pd.DataFrame, list[dict]]:
    """Erstellt den einfachen Tageseditor und trennt geschützte Abwesenheiten."""
    protected_labels = {
        "U": "Urlaub",
        "W": "Wunschfrei",
        "FB": "Fortbildung",
    ...
    for person in employees:
        ...
        code = code_for_person_day(entry, name)
        if code in protected_labels:
            protected_rows.append(
                {"Mitarbeitende": name, "Abwesenheit": protected_labels[code]}
            )
            continue

        allowed_codes = set(erlaubte_dienst_codes(person))
        if str(person.get("nachtdienst", "Nein")) != "Ja":
            allowed_codes.discard("N")
        allowed_text = " / ".join(
            code_value
            for code_value in ("F", "S", "N")
            if code_value in allowed_codes
        ) or "–"
        editable_rows.append(
            {
                "Mitarbeitende": name,
                "Dienst": code if code in {"F", "S", "N", "Frei"} else "",
                "Erlaubt": allowed_text,
            }
        )

    return pd.DataFrame(
        editable_rows,
        columns=["Mitarbeitende", "Dienst", "Erlaubt"],
    ), protected_rows


def manual_day_input_errors(day_table: pd.DataFrame, employees) -> list[str]:
    """Prüft, ob die manuell gewählten Dienste zur Person passen."""
    people_by_name = {
    ...
    for _, row in day_table.iterrows():
        name = str(row.get("Mitarbeitende", "")).strip()
        code = str(row.get("Dienst", "") or "").strip()
        if code not in {"F", "S", "N"}:
            continue
        person = people_by_name.get(name.casefold())
        if not person:
            continue
        if code not in erlaubte_dienst_codes(person):
            errors.append(f"{name}: {code} ist für diese Person nicht erlaubt.")
        elif code == "N" and str(person.get("nachtdienst", "Nein")) != "Ja":
            errors.append(f"{name}: Nachtdienst ist nicht aktiviert.")
    return errors


def apply_manual_day_table(plan, selected_date: str, day_table: pd.DataFrame):
    """Übernimmt F/S/N/Frei für genau einen Tag und schützt Abwesenheiten."""
    ...
    for entry in updated:
        if str(entry.get("Datum", "")) != str(selected_date):
            continue
        assignments = {column: [] for column in code_map.values()}
        for _, row in day_table.iterrows():
            name = str(row.get("Mitarbeitende", "")).strip()
            code = str(row.get("Dienst", "") or "").strip().upper()
            target_column = code_map.get(code)
            if name and target_column:
                assignments[target_column].append(name)
        for column, names in assignments.items():
            entry[column] = join_names(names)
        entry["Status"] = "Tag manuell bearbeitet"
        break
    return updated
...
if page == "Übersicht":
    ...
    if not employees:
        ...
        if not st.session_state.dienstplan_vorlage:
            ...
            with edit_plan_tab:
                st.markdown("#### Manuelle Tagesplanung")
                st.caption(
                    "Wähle einen Tag und trage für jede verfügbare Person F, S, N "
                    "oder Frei ein. Leere Felder bedeuten: nicht eingeplant."
                )

                saved_message = st.session_state.pop("manual_day_saved_message", None)
                if saved_message:
                ...
                day_options = [
                    ...
                    if parse_date(entry.get("Datum"))
                ]
                if st.session_state.get("manual_plan_day") not in day_options:
                    st.session_state.manual_plan_day = day_options[0]
                ...
                selected_date = st.selectbox(
                    ...
                    format_func=lambda value: (
                        f'{value} · {WOCHENTAGE[parse_date(value).weekday()]}'
                        if parse_date(value)
                        else value
                    ),
                    on_change=reset_month_editor_draft,
                )
                selected_entry = next(
                    entry
                    for entry in current_plan
                    if str(entry.get("Datum", "")) == selected_date
                )

                staffing_columns = st.columns(3)
                staffing_values = (
                    ("Frühdienst", "Frühdienst", int(settings.get("min_f", 0))),
                    ("Spätdienst", "Spätdienst", int(settings.get("min_s", 0))),
                    ("Nachtdienst", "Nachtdienst", int(settings.get("min_n", 0))),
                )
                shortages = []
                for metric_column, (label, plan_column, minimum) in zip(
                    staffing_columns,
                    staffing_values,
                ):
                    assigned = len(split_names(selected_entry.get(plan_column, "")))
                    metric_column.metric(label, f"{assigned} / {minimum}")
                    if assigned < minimum:
                        shortages.append(f"{label}: {minimum - assigned} fehlt")
                if shortages:
                    st.warning("Unterbesetzt · " + " · ".join(shortages))
                else:
                    st.success("Die Mindestbesetzung für diesen Tag ist erfüllt.")

                day_table, protected_rows = manual_day_table(selected_entry, employees)
                if protected_rows:
                    protected_text = " · ".join(
                        f'{row["Mitarbeitende"]}: {row["Abwesenheit"]}'
                        for row in protected_rows
                    )
                    st.info("Geschützte Abwesenheiten · " + protected_text)

                with st.form(
                    f"manual_day_form_{st.session_state.dienstplan_editor_version}_{selected_date}"
                ):
                    edited_day_table = st.data_editor(
                        day_table,
                        hide_index=True,
                        width="stretch",
                        num_rows="fixed",
                        disabled=["Mitarbeitende", "Erlaubt"],
                        column_config={
                            "Mitarbeitende": st.column_config.TextColumn(
                                "Mitarbeitende",
                                disabled=True,
                                width="large",
                            ),
                            "Dienst": st.column_config.SelectboxColumn(
                                "Dienst",
                                options=["", "F", "S", "N", "Frei"],
                                required=False,
                                width="medium",
                                help="F = Früh, S = Spät, N = Nacht, Frei = arbeitsfrei",
                            ),
                            "Erlaubt": st.column_config.TextColumn(
                                "Erlaubte Dienste",
                                disabled=True,
                                width="medium",
                            ),
                        ...
                        key=(
                            f"daily_editor_{st.session_state.dienstplan_editor_version}_"
                            f"{selected_date}"
                        ),
                        height=min(700, 105 + len(day_table) * 38),
                    )
                    submit_day = st.form_submit_button(
                        "Tag übernehmen",
                        type="primary",
                        width="stretch",
                    )
                ...
                if submit_day:
                    input_errors = manual_day_input_errors(edited_day_table, employees)
                    candidate_plan = apply_manual_day_table(
                        current_plan,
                        selected_date,
                        edited_day_table,
                    )
                    ...
                    if input_errors:
                        ...
                        st.session_state.auto_plan_result = None
                        st.session_state.manual_day_saved_message = (
                            f"{selected_date} wurde übernommen. Stunden und Besetzung sind aktualisiert."
                        )
                        reset_month_editor_draft()
                        st.rerun()

                st.markdown("#### Aktueller Stundenstand")
                hours_display = current_stats[["Mitarbeitende", "Soll", "Ist"]].copy()
                hours_display["Rest"] = (
                    hours_display["Soll"] - hours_display["Ist"]
                ).round(1)
                st.dataframe(
                    hours_display,
                    hide_index=True,
                    width="stretch",
                    height=min(500, 105 + len(hours_display) * 35),
                )
