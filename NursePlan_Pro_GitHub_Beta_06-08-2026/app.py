...
PAINT_GRID_BUILD = Path(__file__).resolve().parent / "paint_grid" / "dist"
paint_grid_component = components.declare_component(
...
def plan_to_paint_grid(plan, employees) -> tuple[list[dict], list[dict]]:
    ...
    for person in employees:
        ...
        rows.append({"name": name, "cells": cells, "locked": locked})
    return days, rows
    ...
    if not employees:
        ...
        if not st.session_state.dienstplan_vorlage:
            ...
            with edit_plan_tab:
                ...
                paint_days, paint_rows = plan_to_paint_grid(current_plan, employees)
                ...
                st.caption(
                    ...
                    "Soll-, Ist- und Reststunden siehst du nach dem Übernehmen im Reiter "
                    "„Prüfung und Stunden“."
                )
